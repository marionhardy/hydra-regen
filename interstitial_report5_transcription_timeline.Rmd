...
-   It looks like the foot neurons (ec5,ec3A) were not captured in this dataset :(
...
target <- c("NSP4","midasin","mini-COL8","SP-D-like","FH20-X3","CAII",
            ...
            "100208683","100203070","ec5","en1","en2A","en2B") 
...
neurons@meta.data <-
  neurons@meta.data %>%
  mutate(foot_clusters=
           ifelse(seurat_clusters == "0", "ISC/earlyNem",
           ifelse(seurat_clusters == "1", "CELA3B+ midasin+ COL8+", 
           ifelse(seurat_clusters == "2", "ec1A/ec1B myc+",
           ifelse(seurat_clusters == "3", "ec2", 
           ifelse(seurat_clusters == "4", "ec1A",
           ifelse(seurat_clusters == "5", "ec3",
           ifelse(seurat_clusters == "6", "ec3n/ec1n", 
           ifelse(seurat_clusters == "7", "ec4",
           ifelse(seurat_clusters == "8", "TUB4A+ DMRT1+", 
           NA))))))))))

neurons@meta.data <-
  neurons@meta.data %>%
  mutate(head_clusters=
           ifelse(seurat_clusters == "0", "ISC/earlyNem",
           ifelse(seurat_clusters == "1", "CELA3B+ midasin+ COL8+", 
           ifelse(seurat_clusters == "2", "ec1A/ec1B myc+",
           ifelse(seurat_clusters == "3", "ec2", 
           ...
           ifelse(seurat_clusters == "6", "ec3n/ec1n", 
           ifelse(seurat_clusters == "7", "ec4",
           ifelse(seurat_clusters == "8", "ec1B", # that's different from foot neurons
           NA))))))))))
...
# factorising so the colors are the same on the plots

neurons$head_clusters <- factor(neurons$head_clusters, 
                            levels=c("ISC/earlyNem", 
                                     "CELA3B+ midasin+ COL8+", 
                                     "ec1A/ec1B myc+", "ec2", 
                                     "ec1A","ec3", "ec3n/ec1n", 
                                     "ec4", "ec1B"))

neurons$head_clusters <- factor(neurons$foot_clusters, 
                            levels=c("ISC/earlyNem", 
                                     "CELA3B+ midasin+ COL8+", 
                                     "ec1A/ec1B myc+", 
                                     "ec2", "ec1A",  "ec3",
                                     "ec3n/ec1n",  "ec4", "TUB4A+ DMRT1+"))

# re-creating the seperate datasets
...
Idents(hheadf) = "head_clusters"

p1 = DimPlot(hheadf, label = F)+
  labs(title = "Regenerating a head neurons")

Idents(hfootf) = "foot_clusters"

p2 = DimPlot(hfootf, label = F)+
  labs(title = "Regenerating a foot neurons")
...
Please note the behavior of
-   100203070 and others which are OPPOSITE in regenerating a head vs a foot over time
...
aa = table(hheadf$head_clusters, hheadf$Timepoint) 
...
```{r}
ab = table(hfootf$foot_clusters, hfootf$Timepoint) 
...
I feel like I really want to do this per neuron subcluster...

## Neurons subclusters
### Wtf is up with the CELA3B+ COL8+ midasin+ stem cell-like cluster?
